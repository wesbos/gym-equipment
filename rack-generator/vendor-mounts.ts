...
import { isVoltra, vendorAttribution } from './vendor-metadata.ts';
import { validateVoltra, voltraDefaults } from './parts/voltra.ts';
export const isVendorPart = isVoltra;
export function vendorDefaults(part: string): NumericParams { return isVoltra(part) ? {...voltraDefaults} : {}; }
export function vendorPlacement(part: string): PlacementInfo {
 return {family:'voltra',label:vendorAttribution(part)!.credit,paired:true,fields:[],faces:['front','back','left','right'],description:vendorAttribution(part)!.reconstruction};
}
...
export function resolveVendor(doc: RackDoc, accessory: Accessory, targets: Target[]): ResolvedInstance[] {
 return targets.map((t,i) => {
  ...
  const offsets = accessory.part === 'voltra-fixed' ? [-1,1] : [0];
  ...
  const extent = (accessory.params.orientation ?? 1)%2===0 ? 162 : 70;
  return {id:accessory.paired?`${accessory.id}:${i?'right':'left'}`:accessory.id,part:accessory.part,params:{...vendorDefaults(accessory.part),...accessory.params,upright:r.tube,mountSpacing:r.pitch},position:center,rotation:[0,0,angle],mount:mounts[0],mounts,ownerId:accessory.id,kind:'accessory',paired:accessory.paired,connectedTo:[t.uprightId],localOutward:[0,1,0],collisionBoxes:[{min:[-((accessory.params.orientation??1)%2===0?70:162),r.tube/2+45,-extent],max:[(accessory.params.orientation??1)%2===0?70:162,r.tube/2+205,extent]}]};
 });
}
