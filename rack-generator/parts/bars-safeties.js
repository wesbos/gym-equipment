...
function construct(api, p, build) {
  ...
  const allocated = []; const k = x => (allocated.push(x), x);
  const move = (s, v) => k(s.translate(v));
  const rotate = (s, r) => k(s.rotate(r));
  const union = (...s) => k(M.union(s.flat()));
  const cut = (s, ...holes) => k(M.difference([s, ...holes.flat()]));
  const box = (size, center = [0, 0, 0]) => move(k(M.cube(size, true)), center);
  const cyl = (a, b, d, segments = 48) => {
  ...
  const ball = (center, d) => move(k(M.sphere(d / 2, 48)), center);
  const path = (points, d) => union(points.slice(1).map((v, i) => cyl(points[i], v, d)), points.slice(1, -1).map(v => ball(v, d)));
  const roundedProfile = (w, h, r) => k(k(C.square([w - r * 2, h - r * 2], true)).offset(r, 'Round', 2, 48));
  const rounded = (size, r, center = [0, 0, 0]) => move(k(roundedProfile(size[0], size[1], r).extrude(size[2])), [center[0], center[1], center[2] - size[2] / 2]);
  const profileXZ = (points, depth, y = 0) => {
    const area=points.reduce((sum,p,i)=>{const q=points[(i+1)%points.length];return sum+p[0]*q[1]-q[0]*p[1];},0);
    return move(rotate(k(k(new C([area<0?[...points].reverse():points])).extrude(depth)), [90, 0, 0]), [0, y + depth / 2, 0]);
  };
  const plateX = (x, y, z, width, height, thickness = 5, radius = 10, holes = []) => {
  ...
  const ring = (a, b, outside, inside) => cut(cyl(a, b, outside), cyl(a.map((v,i)=>v-(b[i]-a[i])*.1),b.map((v,i)=>v+(b[i]-a[i])*.1),inside));
  // M16 hardware: 30 mm OD washers, 24 mm AF hexagon, 13 mm nut, 115 mm bolt.
  const boltX = (x, y, z, sign, span = 83, diameter = 16) => {
    const pt = t => [x + sign * t, y, z];
  ...
  try {
    result = build({M,C,k,move,rotate,union,cut,box,cyl,ball,path,rounded,roundedProfile,profileXZ,plateX,ring,boltX,capScrew});
    ...
    const keep = new Set(success ? result.map(p => p.solid) : []);
    allocated.reverse().forEach(s => {if(!keep.has(s)) s.delete();});
  }
}
function pullup(kind, api, p) {
  ...
  return construct(api,p,h=>{
    ...
    if(kind==='straight') {
      ...
      const scaled=(x,y,h=z)=>[x*scale,y*sy,h];
...
function saddle(h, x, sign, upright, top, base=0, webbing=false){
