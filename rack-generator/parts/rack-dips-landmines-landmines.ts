...
export function buildRogueLandmine(api: ManifoldAPI, params: NumericParams): SolidPart[] {
  ...
  return vendorSolid(api, s => {
    ...
    if (p.hardware) {
      add('5/8 in band peg', s.keep(s.M.union([K.alongY(0, 0, -fc - 25, y(l1 + 8 + L.bolt / 2), L.bolt / 2, 1, 0, 32), K.alongX(y(l1 + 8 + L.bolt / 2), 0, -L.pegHandle / 2, L.pegHandle / 2, L.bolt / 2, 1.5, 1.5, 32)])), 'fastener', ...ZINC);
...
export function buildMonsterLandmine(api: ManifoldAPI, params: NumericParams): SolidPart[] {
  ...
  return vendorSolid(api, s => {
    ...
    const [cx0, cx1, cz0, cz1] = L.cheek, cheek = K.union2([K.rect(cx0, yc - 24, L.boltX, yc + 24), K.circle(L.boltX, yc, 24, 40)]);
...
export function buildBosLandmine(api: ManifoldAPI, params: NumericParams): SolidPart[] {
  ...
  return vendorSolid(api, s => {
    ...
    if (!p.variant) {
      ...
      add('5/8 in T-handle pin', s.keep(s.M.union([K.alongY(0, 0, -fc - 20, y(94), 7.9, 1, 0, 32), K.alongZ(0, y(94), -B.tHandle / 2, B.tHandle / 2, 7.9, 1.5, 1.5, 32)])), 'source', '#1e1f21', .4, .6);
      ...
      const flange = K.union2([K.rect(fx0, y(l0), B.boltX, y(l1)), K.circle(B.boltX, y((l0 + l1) / 2), (l1 - l0) / 2, 40)]);
      ...
      add('Sleeve hinge tab', K.plateXY(K.union2([K.rect(B.boltX, ty - 24, tx1, ty + 24), K.circle(B.boltX, ty, 24, 40), K.rect(tx0, ty - 24, B.boltX, ty + 24)]), tz0, tz1), 'source', ...BLACK);
    ...
    const cheek = K.union2([K.rect(-m.cheekBack + 12, -z, w, z), K.circle(-m.cheekBack + 12, z - 12, 12, 24), K.circle(-m.cheekBack + 12, -z + 12, 12, 24), K.rect(-m.cheekBack, -z + 12, -m.cheekBack + 12, z - 12)]);
    const cheekCut = s.keep(cheek.subtract(K.circle(0, 0, 13.2, 32)));
    add('Channel bracket', s.keep(s.M.union([box([2 * w, m.plate, 2 * z], [0, fc + m.plate / 2, 0]), K.plateYZ(cheekCut, fc + .5, w), K.plateYZ(cheekCut, -w, -fc - .5)])), 'source', ...BLACK);
    ...
    const ear = K.union2([K.rect(w, -22, hinge, 22), K.circle(hinge, 0, 22, 40)]);
    ...
    const uLeg = K.union2([K.rect(-26, m.uBase[0], 26, m.boltY), K.circle(0, m.boltY, 26, 40)]);
...
export function buildRepLandmine(api: ManifoldAPI, params: NumericParams): SolidPart[] {
  ...
  return vendorSolid(api, s => {
    ...
    add(`${p.series ? '1' : '5/8'} in chrome T-handle pin`, s.keep(s.M.union([K.alongY(0, 0, -fc - 40, tY, d / 2, 2, 0, 40), K.alongZ(0, tY, -R.tHandle / 2, R.tHandle / 2, d / 2, 2, 2, 40)])), 'fastener', ...CHROME);
    ...
    const ear = K.union2([K.rect(y(R.hinge[0] - R.earR), top - 1, y(R.hinge[0] + R.earR), R.hinge[1]), K.circle(y(R.hinge[0]), R.hinge[1], R.earR, 40)]);
...
function buildAdroit(api: ManifoldAPI, p: NumericParams, id: string, rep: boolean): SolidPart[] {
  ...
  return vendorSolid(api, s => {
    ...
    const cheek = K.union2([K.rect(-cw, fc + 24, cw, yc), K.circle(0, yc, cw, 40)]);
