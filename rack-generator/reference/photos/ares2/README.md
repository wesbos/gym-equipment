...
The floating equalizer is reconstructed as adjacent coaxial sheaves at each of two levels, with shared drilled cheeks, axles and spacers. Printed pages 59/67 establish inside D → single fixed upright reversal E → outside F; both floating wraps have the same handedness. Explicit horizontal corner sheaves C and H connect perpendicular cable lanes instead of allowing unsupported cable corners. Printed page 63 shows the outer legs passing separate header fairleads, with the central legs adjacent to E below the flange. Apertures are small bores derived from intersecting cable segments, not blanket beam removals. Coaxial topology and shared support are corrected; only centers, spacing, reversal radius and fabricated support dimensions remain reconstructed estimates.

The lower header is raised to 185 mm for the default profile (first hole + 120 mm), with end webs connecting it to the lower side rails. This is a reconstructed clearance dimension: the manual shows underhung redirects and E above the beam but does not dimension its elevation. Tests check complete wheel/cheek clearance against the beam and guide plates, plus bracket contact with both header and side rails.
