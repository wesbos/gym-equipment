import type { ManifoldAPI, Vec2 } from '../types.ts';
import type { SourceMesh, ConnectedComponent } from './types.ts';
...
export function splitComponents(part: SourceMesh): ConnectedComponent[] {
 const vertices=part.positions,indices=part.indices,n=vertices.length/3;
 const parent=Array.from({length:n},(_,i)=>i),lookup=new Map<string, number>();
...
export function projectProfile(api: ManifoldAPI,component: SourceMesh,axis=0,tolerance=.02): Vec2[][]{
 const axes=[0,1,2].filter(a=>a!==axis),triangles=[];
 for(let i=0;i<component.indices.length;i+=3){const poly=component.indices.slice(i,i+3).map((v): Vec2=>[component.positions[v*3+axes[0]],component.positions[v*3+axes[1]]]);const area=(poly[1][0]-poly[0][0])*(poly[2][1]-poly[0][1])-(poly[1][1]-poly[0][1])*(poly[2][0]-poly[0][0]);if(Math.abs(area)>1e-5){if(area<0)poly.reverse();triangles.push(new api.CrossSection([poly]));}}
