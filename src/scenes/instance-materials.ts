...
export function cloneInstanceMaterials(model: THREE.Group, appearance: Appearance | undefined, instanceId: string) {
  const clone = model.clone();
  clone.traverse(object => {
    if (!(object instanceof THREE.Mesh)) return;
    const source = object.userData.materialSource as MaterialSource;
    object.material = new THREE.MeshStandardMaterial(resolveMaterial(source, appearance, instanceId));
  });
  return clone;
}
