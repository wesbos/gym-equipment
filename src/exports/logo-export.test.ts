...
test('custom nameplate 3MF preserves CAD volume, through-cuts and watertight topology', () => {
  ...
  try {
    ...
    try {
      ...
      assert.ok(Math.abs(exported.volume()-custom[0].solid.volume())<.1, 'custom CAD volume survives XML/Float32 and print orientation');
      assert.ok(Math.abs(exported.volume()-stock[0].solid.volume())>1000, 'export did not silently use stock lettering');
