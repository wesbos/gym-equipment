...
test('3MF partition removes overlapping volume, preserves colors and physical assembly',()=>{
  ...
  assert.equal(archive.byId.get(archive.items[0].objectid)?.name,'front-left · Test & "quoted" <part>');
  ...
  try {assert.equal(overlap.volume(),0);assert.equal(solids.reduce((s,m)=>s+m.volume(),0),6000);} finally {overlap.delete();solids.forEach(s=>s.delete());}
  result.report.parts[0].size.forEach((v,i)=>assert.ok(Math.abs(v-[30,20,10][i])<1e-8));
});
...
test('assembly coordinates are baked in millimetres and layout leaves input unchanged',()=>{
  ...
  assert.equal(Math.min(...coords.map(v=>v[0])),entry.position[0]);
  assert.equal(Math.min(...coords.map(v=>v[1])),entry.position[1]);
...
test('complete rack XML carries exact duplication, per-side overrides, dimensions and slicer materials',()=>{
  ...
  assert.equal(archive.items.length,entries.length);assert.equal(result.report.instances,14);
  assert.equal(result.report.parts.filter(p=>p.part==='upright').length,4);
  for(const p of result.report.parts.filter(p=>p.part==='upright')) assert.ok(Math.abs(Math.max(...p.size)-2032)<.001);
  ...
  assert.equal(settings.config.object.length,entries.length);
  ...
  for(const item of archive.items) {
    const x=Number(item.transform!.split(' ')[9]);assert.ok(x>=previousMax+20-.000001);
    const meshObjects=list(archive.byId.get(item.objectid)!.components!.component).map(c=>archive.byId.get(c.objectid)!);
    const vertices=meshObjects.flatMap(o=>o.mesh!.vertices.vertex);
    assert.ok(Math.abs(Math.min(...vertices.map(v=>+v.z)))<.00001);
    previousMax=x+Math.max(...vertices.map(v=>+v.x));
  }
