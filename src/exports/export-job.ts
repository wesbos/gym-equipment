...
import type { PrintLayout, PrintScale } from './print-3mf.ts';
...
export class ExportJob {
  ...
  async start(format: ExportFormat, doc: RackDoc, layout: PrintLayout, scale: PrintScale = 10) {
    ...
    try {
      ...
      worker.onmessage = ({ data }: MessageEvent<PrintResponse>) => {
        ...
        try {
          this.deps.download(new Blob([data.bytes], { type: 'model/3mf' }), `bos-strength-print-parts-1-${data.report.scale}.3mf`);
          finish();
          this.deps.status(`3MF exported: ${data.report.instances} parts, ${data.report.volumes} watertight volumes · millimetres, 1:${data.report.scale} scale.${data.report.occludedComponents.length ? ` ${data.report.occludedComponents.length} fully covered components omitted (see archive report).` : ''}`);
          this.deps.complete();
        } catch (error) { fail(error); }
      };
      worker.postMessage({ doc: structuredClone(doc), options: { layout, scale } } satisfies PrintRequest);
    } catch (error) { fail(error); }
  }
