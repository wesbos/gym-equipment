 ...
 *
 *   npx tsx scripts/render-gym-previews.ts [--port 5661] [--base http://127.0.0.1:5661] [--view iso] [slug …]
 *
 * With no slugs it renders every src/gyms/data/<slug>.json. Each gym opens through the builder's own
...
const port = Number(flag('port', '5661')), view = flag('view', 'iso'), external = flag('base', '');
const base = external || `http://127.0.0.1:${port}`;
const WIDTH = 1200, HEIGHT = 800, MAX_BYTES = 150 * 1024;
const root = fileURLToPath(new URL('../', import.meta.url));
const slugs = args.length ? args : readdirSync(`${root}src/gyms/data`).filter(f => f.endsWith('.json')).map(f => f.replace(/\.json$/, '')).sort();
...
try {
  for (const slug of slugs) {
    ...
    try {
      ...
      await page.waitForFunction(() => !new URLSearchParams(location.search).has('gym'), undefined, { timeout: 60000 });
      await settle(page);
      ...
      console.log(`${slug}: ${status} → public/gyms/${slug}.webp (${Math.round(bytes.length / 1024)} KB, q${quality})`);
    } catch (error) {
      failed++;
